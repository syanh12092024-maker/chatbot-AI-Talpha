...
| `PK_MARK_UNREAD` | Sau khi bot GỬI tin cho khách, gọi `/unread` để hội thoại quay lại hàng chờ sale. Pancake coi hội thoại là đã đọc ngay khi page gửi, kể cả tin bot — không gọi thì bot trả lời xong là sale không thấy khách đó nữa. **Cùng một cờ với đường v1** (`pancake-poll.js:539`), nay v3 dùng chung. `0` = tắt | vắng = BẬT | để mặc định | L2-M1 · v1 |
...
| `V3_GIAO_PAGE_TREN_MAN` | **CẦU DAO «giao page bằng giao diện»** (024 · 25/09). `=1` ⇒ nguồn của «page này thuộc bot nào» là cột `page.giao_bot_moi` (sửa được trên màn «Công tắc từng page»), và danh sách chủ sở hữu là **HỢP** của cột ấy với `V3_PAGE_XU_LY` — page đã có tên trong cấu hình máy chủ thì bot cũ ĐÃ tránh ra, bỏ nó khỏi tay bot mới là để một page không con nào trả lời. PHANH TAY lúc sự cố là **tắt chính cầu dao này** rồi khởi động lại: chủ sở hữu quay về đúng danh sách trong cấu hình máy chủ (page vừa giao bằng giao diện sẽ không ai trả lời — im lặng, hướng hỏng an toàn). Vắng ⇒ y hệt hôm nay: nguồn là `V3_PAGE_XU_LY`, cột nằm im. ⚠️ Bot CŨ không đọc cột này; thứ khiến nó buông một page giao bằng giao diện là công tắc AI của chính nó, nên cửa giao (`v3/src/ui/page-bot/cong-tac.js#giaoPage`) BẮT BUỘC tắt bot cũ rồi ĐỌC LẠI xác nhận trước khi ghi cờ. Bật cầu dao MỘT LẦN là đủ — sau đó không cần SSH nữa | vắng = đóng | `1` sau khi thử giao 1 page có người canh (`mo-van` bậc ③) | 024 · Q1 · GD2 |
