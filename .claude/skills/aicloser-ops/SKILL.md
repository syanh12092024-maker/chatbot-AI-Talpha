...
- **Hỏi chi phí token** → `/admin/api/token-cost?from&to` — số ĐO thật (event `reply` trong Sổ AI mang `tin/tout/cread/calls` từ 06/08/2026; classifier + closer đều tính). Quy tiền theo `config.aiPrices` (kimi-k2.6: $0.95 vào/$0.16 cache/$4 ra; haiku: $1/$0.1/$5 — đè bằng `AI_PRICE_IN/CACHE/OUT`, tỉ giá `AI_USD_VND`). Dashboard có thẻ "Chi phí AI (đo)" + cột "Chi phí AI" theo page. Tin trước 06/08 không có số đo (`measured` < `replies`).
