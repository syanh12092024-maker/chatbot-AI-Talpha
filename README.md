...
8. **Cầu chì an toàn — chống spam khách** — tối đa **5 lượt AI/khách trong 24h** (`MAX_AI_TURNS`), đếm BỀN từ Sổ AI nên restart server không "reset chui" thêm lượt; khách nhắn dồn nhiều tin liên tiếp → AI **đợi khách gõ xong ~20s** (`REPLY_DEBOUNCE_MS`) rồi trả lời **1 lần cho cả cụm**, không đáp riêng từng tin; `maxToolIterations` (5) vòng tool/lượt; classifier lỗi → fallback an toàn. (`config.js`, `ai-log.js → recentReplyCount`, `pancake-poll.js`)
