...
2. **Trung thực thông tin** — giá/chính sách chỉ lấy từ KB hoặc tool `get_price`, không bịa; mỗi page 1 sản phẩm; luôn coi còn hàng; chủ động gửi ảnh thật (`send_product_image`). **Lượt AI đầu tiên với mỗi khách do CODE ép gửi ảnh** (`IMG_FIRST_TURN`, mặc định 2 ảnh) — lời dặn trong prompt chỉ được Haiku nghe ~49% số lần (đo trên 1045 khách/7 ngày), có page 12 ảnh mà chưa gửi cho khách nào. AI tự gửi rồi thì không ép chồng. (`handler.js → ensureFirstTurnImages`)
