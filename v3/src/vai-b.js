 ...
 */
export function dungPhanB(app, { taoTruyVan, taoTruyVanHeThong, ghiSoAi, canhBao, express } = {}) {
  ...
  daNoi.push('cổng dữ liệu → nhật ký · lớp model · bảng điều phối · kho người dùng');
  ...
  for (const dat of [datPheuNhatKyAuth, datPheuNhatKyModel, datPheuNhatKyDieuPhoi]) {
    dat((boiCanh, ban) => {
  ...
  daNoi.push('nhật ký ← đăng nhập · lớp model · bảng điều phối');
  ...
  daNoi.push('chắn đăng nhập + chắn vai → bảng điều phối');
  ...
  daNoi.push('router: bối cảnh → đăng nhập → chặn xuyên team → điều phối');
