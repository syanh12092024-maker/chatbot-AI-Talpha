// ĐƯỜNG HTTP CỦA MÀN «CHI PHÍ AI» (G2-G2).
//
// | GET /chi-phi      | trang                                                     |
// | GET /api/chi-phi  | tiền theo page + chỗ đốt tiền không ra đơn + lệch hai sổ  |
//
// MÀN CHỈ ĐỌC.
...
import { manChiPhi, VAI_VAO_DUOC, NGUON, LoiChiPhi } from './kho-chi-phi.js';

const THU_MUC = path.dirname(fileURLToPath(import.meta.url));
const TRANG = (ten) => path.join(THU_MUC, 'trang', ten);

export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/chi-phi';
...
export const daNoiChanChiPhi = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[chi-phi] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
...
function chanHong(ten, e, res) {
  console.error(`[chi-phi] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[chi-phi] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterChiPhi() {
...
<div class="h"><h1>Không đủ quyền xem chi phí AI</h1><p>${escHtml(cau)}</p>
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
      }
      return res.status(403).json({ ok: false, ma: 'thieu_vai', thongDiep: cau });
    }
    return res.sendFile(TRANG('chi-phi.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/chi-phi', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await manChiPhi(cuaBoiCanh(req))) });
  }));

  return r;
}

export { NGUON, LoiChiPhi };
