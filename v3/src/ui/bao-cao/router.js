// ĐƯỜNG HTTP CỦA MÀN «BÁO CÁO» (G2-G1).
//
// | GET /bao-cao      | trang                                                    |
// | GET /api/bao-cao  | luồng Messenger theo BA thước + luồng trang bán hàng      |
//
// MÀN CHỈ ĐỌC.
...
export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/bao-cao';
...
export const daNoiChanBaoCao = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[bao-cao] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
...
function chanHong(ten, e, res) {
  console.error(`[bao-cao] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[bao-cao] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterBaoCao() {
...
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
    ...
    return res.sendFile(TRANG('bao-cao.html'), (e) => (e ? next(e) : undefined));
  });
