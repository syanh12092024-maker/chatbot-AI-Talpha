// ĐƯỜNG HTTP CỦA MÀN «NGUỒN KHÁCH VÀO» (G2-G4).
//
// | GET /nguon-khach      | trang                                                |
// | GET /api/nguon-khach  | hai luồng đơn + phễu hội thoại + chỗ rơi WhatsApp    |
//
// MÀN CHỈ ĐỌC.
...
import { manNguon, VAI_VAO_DUOC, BAC, LoiNguon } from './kho-nguon.js';

const THU_MUC = path.dirname(fileURLToPath(import.meta.url));
const TRANG = (ten) => path.join(THU_MUC, 'trang', ten);

export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/nguon-khach';
...
export const daNoiChanNguon = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[nguon-khach] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
...
function chanHong(ten, e, res) {
  console.error(`[nguon-khach] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[nguon-khach] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterNguon() {
...
<div class="h"><h1>Không đủ quyền xem nguồn khách</h1><p>${escHtml(cau)}</p>
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
      }
      return res.status(403).json({ ok: false, ma: 'thieu_vai', thongDiep: cau });
    }
    return res.sendFile(TRANG('nguon-khach.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/nguon-khach', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await manNguon(cuaBoiCanh(req))) });
  }));
