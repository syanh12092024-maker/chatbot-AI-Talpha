// ĐƯỜNG HTTP CỦA MÀN «Prompt của page».
//
// | GET /prompt-page             | trang                                                 |
// | GET /api/prompt-page/danh-sach| page chọn được                                        |
// | GET /api/prompt-page/:pageId | bốn khối + token từng khối + chỗ đáng đọc lại         |
//
// MÀN NÀY CHỈ ĐỌC. Không có đường ghi nào — sửa khối nào thì sang đúng màn của khối đó.
...
import {
  promptCua, pageChonDuoc, LoiPrompt,
} from './kho-prompt.js';
...
export const DUONG_TRANG = '/prompt-page';
...
export const daNoiChanPromptPage = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[prompt-page] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
    return res.status(500).json({ ok: false, ma: 'chua_noi_chan', thongDiep: 'Máy chủ chưa nối lớp đăng nhập cho Prompt của page.' });
  };
}

function chanHong(ten, e, res) {
  console.error(`[prompt-page] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[prompt-page] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterPromptPage() {
  ...
  r.get(DUONG_TRANG, (req, res, next) => {
    ...
    if (!coVai(bc, ...VAI_VAO_DUOC)) {
      const cau = `Màn Prompt của page cần một trong các vai: ${VAI_VAO_DUOC.join(', ')}. `
...
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
    ...
    return res.sendFile(TRANG('prompt-page.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/prompt-page/danh-sach', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await pageChonDuoc(cuaBoiCanh(req), { tim: req.query.tim || '' })) });
  }));

  r.get('/api/prompt-page/:pageId', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await promptCua(cuaBoiCanh(req), req.params.pageId)) });
  }));
