// ĐƯỜNG HTTP CỦA MÀN «CỬA KIỂM SẴN SÀNG» (G2-F5).
//
// | GET /san-pham      | trang                                                    |
// | GET /api/san-pham  | page của team + bảy điều kiện + chỗ lệch với CSDL        |
//
// MÀN CHỈ ĐỌC — không có cửa ghi nào, nên không có `VAI_SUA_DUOC`. Sửa thì bấm sang màn
// tương ứng: đó là cả ý của «bấm ô đỏ nhảy thẳng tới chỗ sửa». Dựng thêm nút sửa tại chỗ ở
// đây là dựng bản thứ hai của bốn màn khác.
...
export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/san-pham';
...
export const daNoiChanSanPham = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[san-pham] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
...
function chanHong(ten, e, res) {
  console.error(`[san-pham] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
export function taoRouterSanPham() {
  ...
  r.get(DUONG_TRANG, (req, res, next) => {
    ...
    if (!coVai(bc, ...VAI_VAO_DUOC)) {
      const cau = `Màn Sản phẩm & kho cần một trong các vai: ${VAI_VAO_DUOC.join(', ')}. `
...
<div class="h"><h1>Không đủ quyền xem kho sản phẩm</h1><p>${escHtml(cau)}</p>
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
      }
      return res.status(403).json({ ok: false, ma: 'thieu_vai', thongDiep: cau });
    }
    return res.sendFile(TRANG('san-pham.html'), (e) => (e ? next(e) : undefined));
  });
  ...
  r.get('/api/san-pham/:id', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await sanPhamCuaMotPage(cuaBoiCanh(req), req.params.id)) });
  }));

  return r;
}

export { VI_RONG, LoiSanPham };
