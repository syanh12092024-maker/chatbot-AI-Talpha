// ĐƯỜNG HTTP CỦA MÀN «Thư viện kỹ năng».
//
// | GET  /ky-nang                 | trang                                                |
// | GET  /api/ky-nang             | thư viện + số page THẬT SỰ nhận mỗi kỹ năng          |
// | POST /api/ky-nang/:id/bat     | bật/tắt một kỹ năng                                  |
// | POST /api/ky-nang/:id/nhom    | khoanh nhóm sản phẩm                                 |
//
// Marketer sửa được — kỹ năng là tầng của họ (`01-QUYET-DINH.md` §6 "bật theo sản phẩm").
...
import {
  manKyNang, batTatKyNang, datNhomSanPham, LoiKyNang,
} from './kho-ky-nang.js';

const THU_MUC = path.dirname(fileURLToPath(import.meta.url));
const TRANG = (ten) => path.join(THU_MUC, 'trang', ten);

export const VAI_VAO_DUOC = Object.freeze([VAI.QUAN_TRI, VAI.MARKETER, VAI.QUAN_LY]);
export const VAI_SUA_DUOC = Object.freeze([VAI.QUAN_TRI, VAI.MARKETER]);
export const DUONG_TRANG = '/ky-nang';
...
export const daNoiChanKyNang = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[ky-nang] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
    return res.status(500).json({ ok: false, ma: 'chua_noi_chan', thongDiep: 'Máy chủ chưa nối lớp đăng nhập cho Thư viện kỹ năng.' });
  };
}

function chanHong(ten, e, res) {
  console.error(`[ky-nang] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function chanGhiMw(req, res, next) {
  ...
  if (!coVai(bc, ...VAI_SUA_DUOC)) {
    return res.status(403).json({
      ...
      thongDiep: `Chỉ vai ${VAI_SUA_DUOC.join(', ')} sửa được. Vai của bạn: ${bc.vai.join(', ') || 'không có'}.`,
    });
  }
  return next();
}
...
function traLoi(res, e) {
  ...
  console.error('[ky-nang] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterKyNang() {
  ...
  r.get(DUONG_TRANG, (req, res, next) => {
    ...
    if (!coVai(bc, ...VAI_VAO_DUOC)) {
      const cau = `Màn Thư viện kỹ năng cần một trong các vai: ${VAI_VAO_DUOC.join(', ')}. `
...
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
    ...
    return res.sendFile(TRANG('ky-nang.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/ky-nang', canDangNhap, canVai, boc(async (req, res) => {
    const bc = cuaBoiCanh(req);
    res.json({ ok: true, ...(await manKyNang(bc)), suaDuoc: coVai(bc, ...VAI_SUA_DUOC) });
  }));

  r.post('/api/ky-nang/:id/bat', canDangNhap, canVai, chanGhiMw, boc(async (req, res) => {
    const bat = req.body?.bat === true || req.body?.bat === 'true' || req.body?.bat === 1;
    res.json({ ok: true, ...(await batTatKyNang(cuaBoiCanh(req), req.params.id, bat)) });
  }));

  r.post('/api/ky-nang/:id/nhom', canDangNhap, canVai, chanGhiMw, boc(async (req, res) => {
    res.json({ ok: true, ...(await datNhomSanPham(cuaBoiCanh(req), req.params.id, req.body?.nhom)) });
  }));
