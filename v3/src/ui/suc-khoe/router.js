...
import { bangDen, LoiSucKhoe } from './kho-suc-khoe.js';

const THU_MUC = path.dirname(fileURLToPath(import.meta.url));
const TRANG = (ten) => path.join(THU_MUC, 'trang', ten);

/** Bốn vai vào được — sức khoẻ hệ thống là thứ ai cũng nên nhìn thấy. Sale thì không: họ
 *  chỉ làm việc ở bảng điều phối (01-QUYET-DINH §10). */
export const VAI_VAO_DUOC = Object.freeze([VAI.QUAN_TRI, VAI.QUAN_LY, VAI.MARKETER, VAI.DUYET_KICH_BAN]);
export const DUONG_TRANG = '/suc-khoe';
...
export const daNoiChanSucKhoe = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[suc-khoe] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
    return res.status(500).json({ ok: false, ma: 'chua_noi_chan', thongDiep: 'Máy chủ chưa nối lớp đăng nhập cho Sức khoẻ hệ thống.' });
  };
}

function chanHong(ten, e, res) {
  console.error(`[suc-khoe] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[suc-khoe] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterSucKhoe() {
  ...
  r.get(DUONG_TRANG, (req, res, next) => {
    ...
    if (!coVai(bc, ...VAI_VAO_DUOC)) {
      const cau = `Màn Sức khoẻ hệ thống cần một trong các vai: ${VAI_VAO_DUOC.join(', ')}. `
...
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
    ...
    return res.sendFile(TRANG('suc-khoe.html'), (e) => (e ? next(e) : undefined));
  });

  r.get('/api/suc-khoe', chanDangNhapMw(), chanVaiMw(), boc(async (req, res) => {
    res.json({ ok: true, ...(await bangDen(cuaBoiCanh(req))) });
  }));
