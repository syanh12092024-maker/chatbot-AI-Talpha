// ĐƯỜNG HTTP CỦA MÀN «RỦI RO HOÀN HÀNG» (G2-G7).
//
// | GET /rui-ro-hoan      | trang                                                |
// | GET /api/rui-ro-hoan  | phân bố tỉ lệ hoàn × số đơn, và chính sách chưa chốt |
//
// MÀN CHỈ ĐỌC — và cố ý không có cửa nào xếp tầng cho khách: bốn tầng đang «chờ chốt».
...
export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/rui-ro-hoan';
...
export const daNoiChanRuiRo = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[rui-ro-hoan] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
...
function chanHong(ten, e, res) {
  console.error(`[rui-ro-hoan] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[rui-ro-hoan] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterRuiRo() {
...
<div class="h"><h1>Không đủ quyền xem rủi ro hoàn</h1><p>${escHtml(cau)}</p>
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
      }
      return res.status(403).json({ ok: false, ma: 'thieu_vai', thongDiep: cau });
    }
    return res.sendFile(TRANG('rui-ro-hoan.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/rui-ro-hoan', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await manRuiRo(cuaBoiCanh(req))) });
  }));
