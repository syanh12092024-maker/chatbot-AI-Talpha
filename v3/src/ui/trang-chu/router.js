// ĐƯỜNG HTTP CỦA MÀN «TRANG CHỦ» (G2-F1).
//
// | GET /trang-chu      | trang                                       |
// | GET /api/trang-chu  | danh sách việc ĐÃ LỌC THEO VAI của người xem |
//
// MÀN CHỈ ĐỌC — mọi việc đều bấm sang màn khác để làm. Không cửa ghi, nên không `VAI_SUA_DUOC`.
//
// MỌI VAI ĐỀU VÀO ĐƯỢC, kể cả `sale`: đây là màn đầu tiên người ta mở. Cái quyết định người
// ta thấy gì là bộ lọc theo vai ở tầng dưới, không phải cái chắn ở cửa.
...
import { manTrangChu, VAI_VAO_DUOC, VI_RONG, LoiTrangChu } from './kho-trang-chu.js';

const THU_MUC = path.dirname(fileURLToPath(import.meta.url));
const TRANG = (ten) => path.join(THU_MUC, 'trang', ten);

export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/trang-chu';
...
export const daNoiChanTrangChu = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[trang-chu] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
    return res.status(500).json({ ok: false, ma: 'chua_noi_chan', thongDiep: 'Máy chủ chưa nối lớp đăng nhập cho Trang chủ.' });
  };
}

function chanHong(ten, e, res) {
  console.error(`[trang-chu] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[trang-chu] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterTrangChu() {
  ...
  r.get(DUONG_TRANG, (req, res, next) => {
    ...
    if (!coVai(bc, ...VAI_VAO_DUOC)) {
      // Không có VAI NÀO — khác hẳn «có vai nhưng không đủ»: người này chưa được xếp vào đâu.
      const cau = 'Tài khoản của bạn chưa được gán vai nào trong team này, nên chưa có việc '
        + 'nào để hiện. Nhờ quản trị gán vai ở màn Cấu hình team.';
      if (muonTrang(req)) {
        return res.status(403).send(`<!doctype html><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1"><title>Chưa có vai</title>
...
h1{font-size:16px;margin:0 0 8px}p{margin:0 0 14px;color:#475467;line-height:1.55}</style>
<div class="h"><h1>Chưa được gán vai</h1><p>${escHtml(cau)}</p></div>`);
      }
      return res.status(403).json({ ok: false, ma: 'thieu_vai', thongDiep: cau });
    }
    return res.sendFile(TRANG('trang-chu.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/trang-chu', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await manTrangChu(cuaBoiCanh(req))) });
  }));

  return r;
}

export { VI_RONG, LoiTrangChu };
