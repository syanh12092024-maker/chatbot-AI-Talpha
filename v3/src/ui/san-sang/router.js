...
//
// | GET /san-sang      | trang                                                    |
// | GET /api/san-sang  | page của team + bảy điều kiện + chỗ lệch với CSDL        |
//
...
export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/san-sang';
...
export const daNoiChanSanSang = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[san-sang] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
...
function chanHong(ten, e, res) {
  console.error(`[san-sang] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[san-sang] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterSanSang() {
...
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
    ...
    return res.sendFile(TRANG('san-sang.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/san-sang', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await manSanSang(cuaBoiCanh(req))) });
  }));

  return r;
}

export { DIEU_KIEN, LoiSanSang };
