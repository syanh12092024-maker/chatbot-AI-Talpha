// ĐƯỜNG HTTP CỦA MÀN «CÀI ĐẶT TEAM» (GD3 · 25/09/2026).
//
// | GET /cai-dat-team      | trang                                                |
// | GET /api/cai-dat-team  | năm bước + việc phải nhờ người quản trị hệ thống      |
//
// MÀN CHỈ ĐỌC — không một cửa ghi nào. Mỗi bước dẫn sang màn vốn đã có cửa ghi, lớp vai và
// nhật ký của nó. Dựng cửa ghi thứ hai ở một màn tổng quan là cách chắc chắn nhất để hai
// cửa trôi khỏi nhau.
...
export const daNoiChanCaiDat = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[cai-dat-team] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
    return res.status(500).json({ ok: false, ma: 'chua_noi_chan', thongDiep: 'Máy chủ chưa nối lớp đăng nhập cho màn Cài đặt team.' });
  };
}

function chanHong(ten, e, res) {
  console.error(`[cai-dat-team] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[cai-dat-team] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterCaiDat() {
  ...
  r.get(DUONG_TRANG, (req, res, next) => {
    ...
    if (!coVai(bc, ...VAI_VAO_DUOC)) {
      const cau = `Màn Cài đặt team cần một trong các vai: ${VAI_VAO_DUOC.join(', ')}. `
...
<div class="h"><h1>Không đủ quyền xem màn Cài đặt team</h1><p>${escHtml(cau)}</p>
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
      }
      return res.status(403).json({ ok: false, ma: 'thieu_vai', thongDiep: cau });
    }
    return res.sendFile(TRANG('cai-dat-team.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/cai-dat-team', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await manCaiDat(cuaBoiCanh(req))) });
  }));
