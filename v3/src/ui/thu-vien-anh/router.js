// ĐƯỜNG HTTP CỦA MÀN «THƯ VIỆN ẢNH» (G2-D5).
//
// | GET /thu-vien-anh      | trang                                                  |
// | GET /api/thu-vien-anh  | ảnh của page trong team, theo trang, + có chủ đề chưa  |
//
// MÀN CHỈ ĐỌC. Gắn nhãn chủ đề là ghi ngược vào Google Sheet — v3 chưa có cửa ghi sang đó,
// và dựng một cửa nửa vời sẽ đẻ ra nguồn ảnh thứ hai.
//
// ĐỌC THEO TRANG vì mỗi page là một lời gọi HTTP sang tiến trình bot.
...
import { manAnh, VAI_VAO_DUOC, LoiAnh } from './kho-anh.js';

const THU_MUC = path.dirname(fileURLToPath(import.meta.url));
const TRANG = (ten) => path.join(THU_MUC, 'trang', ten);

export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/thu-vien-anh';
...
export const daNoiChanAnh = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[thu-vien-anh] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
    return res.status(500).json({ ok: false, ma: 'chua_noi_chan', thongDiep: 'Máy chủ chưa nối lớp đăng nhập cho màn Thư viện ảnh.' });
  };
}

function chanHong(ten, e, res) {
  console.error(`[thu-vien-anh] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[thu-vien-anh] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterAnh() {
  ...
  r.get(DUONG_TRANG, (req, res, next) => {
    ...
    if (!coVai(bc, ...VAI_VAO_DUOC)) {
      const cau = `Màn Thư viện ảnh cần một trong các vai: ${VAI_VAO_DUOC.join(', ')}. `
...
<div class="h"><h1>Không đủ quyền xem thư viện ảnh</h1><p>${escHtml(cau)}</p>
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
      }
      return res.status(403).json({ ok: false, ma: 'thieu_vai', thongDiep: cau });
    }
    return res.sendFile(TRANG('thu-vien-anh.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/thu-vien-anh', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await manAnh(cuaBoiCanh(req), { trang: req.query.trang })) });
  }));
