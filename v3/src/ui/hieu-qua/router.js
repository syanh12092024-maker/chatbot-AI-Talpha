// ĐƯỜNG HTTP CỦA MÀN «HIỆU QUẢ KỊCH BẢN».
//
// | GET /hieu-qua      | trang     |
// | GET /api/hieu-qua  | dữ liệu   |
//
// MÀN CHỈ ĐỌC.
...
import { manHieuQua, VAI_VAO_DUOC, LoiHieuQua } from './kho-hieu-qua.js';

const THU_MUC = path.dirname(fileURLToPath(import.meta.url));
const TRANG = (ten) => path.join(THU_MUC, 'trang', ten);

export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/hieu-qua';
...
export const daNoiChanHieuQua = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[hieu-qua] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
...
function chanHong(ten, e, res) {
  console.error(`[hieu-qua] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[hieu-qua] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterHieuQua() {
...
<div class="h"><h1>Không đủ quyền xem hiệu quả kịch bản</h1><p>${escHtml(cau)}</p>
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
      }
      return res.status(403).json({ ok: false, ma: 'thieu_vai', thongDiep: cau });
    }
    return res.sendFile(TRANG('hieu-qua.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/hieu-qua', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await manHieuQua(cuaBoiCanh(req))) });
  }));
