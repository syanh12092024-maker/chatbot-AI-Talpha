// ĐƯỜNG HTTP CỦA MÀN «ĐƯA SẢN PHẨM MỚI LÊN CHẠY» (G2-F7).
//
// | GET /len-chay          | trang                                            |
// | GET /api/len-chay      | danh sách page + vì sao chặng 2 chặn cả team     |
// | GET /api/len-chay/:id  | sáu chặng của MỘT page, kèm chặng đang dừng      |
//
// MÀN CHỈ ĐỌC. Mỗi chặng có nút bấm sang màn LÀM chặng đó — dựng lại việc của sáu màn khác
// ở đây là dựng sáu bản thứ hai.
//
// Page của team khác → **404**, không phải 403.
...
import { manLenChay, changCuaPage, VAI_VAO_DUOC, CHANG, LoiLenChay } from './kho-len-chay.js';

const THU_MUC = path.dirname(fileURLToPath(import.meta.url));
const TRANG = (ten) => path.join(THU_MUC, 'trang', ten);

export { VAI_VAO_DUOC };
export const DUONG_TRANG = '/len-chay';
...
export const daNoiChanLenChay = () => typeof _chanDangNhap === 'function' && typeof _chanVai === 'function';

function chanChuaNoi(ten) {
  return (_req, res) => {
    console.error(`[len-chay] chưa nối ${ten} lúc dựng ứng dụng. Chặn để an toàn.`);
    return res.status(500).json({ ok: false, ma: 'chua_noi_chan', thongDiep: 'Máy chủ chưa nối lớp đăng nhập cho màn Sản phẩm & kho.' });
  };
}

function chanHong(ten, e, res) {
  console.error(`[len-chay] cái chắn ${ten} ném lỗi:`, e?.stack || e?.message || e);
...
function traLoi(res, e) {
  ...
  console.error('[len-chay] lỗi chưa phân loại:', e?.stack || e?.message || e);
...
export function taoRouterLenChay() {
  ...
  r.get(DUONG_TRANG, (req, res, next) => {
    ...
    if (!coVai(bc, ...VAI_VAO_DUOC)) {
      const cau = `Màn Đưa sản phẩm mới lên chạy cần một trong các vai: ${VAI_VAO_DUOC.join(', ')}. `
...
<div class="h"><h1>Không đủ quyền xem chặng lên chạy</h1><p>${escHtml(cau)}</p>
<p><a href="/dieu-phoi">← Về bảng điều phối</a></p></div>`);
      }
      return res.status(403).json({ ok: false, ma: 'thieu_vai', thongDiep: cau });
    }
    return res.sendFile(TRANG('len-chay.html'), (e) => (e ? next(e) : undefined));
  });

  const canDangNhap = chanDangNhapMw();
  const canVai = chanVaiMw();

  r.get('/api/len-chay', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await manLenChay(cuaBoiCanh(req))) });
  }));

  r.get('/api/len-chay/:id', canDangNhap, canVai, boc(async (req, res) => {
    res.json({ ok: true, ...(await changCuaPage(cuaBoiCanh(req), req.params.id)) });
  }));

  return r;
}

export { CHANG, LoiLenChay };
