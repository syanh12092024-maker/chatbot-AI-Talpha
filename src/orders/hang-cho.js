...
/**
 * Quy `tong_tien_lon` (đơn vị LỚN, khuôn cũ bộ não) sang `tong_tien` (đơn vị NHỎ POS)
 * khi đã biết tệ — `HE_SO_TE` nhân ĐÚNG MỘT LẦN, ở đây và chỉ ở đây (RF-9). Chưa biết tệ
 * hoặc tệ lạ ⇒ `tong_tien` giữ null (cửa ① báo thiếu — mù CÓ NÓI RA, không đoán ×100).
 * Gọi lại ở `duyet()` sau khi gộp `boSung`: sale bổ sung `tien_te` là đủ để quy.
 */
export function quyTongTienNho(duLieu = {}) {
  const d = { ...duLieu };
 ...
 *
 * ĐƠN VỊ TIỀN (RF-9) — khai theo TÊN KHOÁ, một nguồn:
 *   · `tong_tien` (tên cột v3)            = đơn vị NHỎ POS, cùng đơn vị `goi_gia.gia`
 *                                           (migration 007 COMMENT) — giữ nguyên.
 *   · `total_price` / `total` (khuôn cũ)   = đơn vị LỚN («99 nghĩa là 99 SAR/AED» —
 *                                           `src/tools.js:38`, file cấm sửa) ⇒ giữ ở
 *                                           `tong_tien_lon`, quy sang nhỏ bằng
 *                                           `quyTongTienNho` khi biết tệ.
 * Bộ não KHÔNG trả `currency` (đo: `grep currency src/context.js` = 0 dòng trong prof)
 * ⇒ dòng bot chốt thường vào hàng chờ với `tong_tien=null` + `tong_tien_lon=<số>`; sale
 * bổ sung `tien_te` lúc duyệt thì `duyet()` quy lại. Cửa ② so `tong_tien` (nhỏ) với
 * `goi_gia.gia` (nhỏ) — cùng đơn vị, không còn 15 so với 1500.
 */
export function chuanHoaHoSo(hoSo = {}) {
  const h = hoSo || {};
  const coV3 = h.tong_tien != null && h.tong_tien !== "";
  return quyTongTienNho({
    ...
    tong_tien: coV3 ? so(h.tong_tien) : null,
    tong_tien_lon: coV3 ? so(h.tong_tien_lon) : so(h.total_price ?? h.total),
...
export async function duyet(
  ...
  try {
    ...
    // RF-9: sau khi gộp bổ sung (sale có thể vừa cho `tien_te`) quy `tong_tien_lon` →
    // `tong_tien` (nhỏ) — HE_SO_TE nhân đúng MỘT lần, chỉ khi `tong_tien` còn null.
    const duLieu = quyTongTienNho(
      boSung ? { ...duLieuCu, ...chuanHoaHoSoBoSung(boSung) } : { ...duLieuCu },
    );
